 ...
 * default to U.S. Standard (english) unless Metric is chosen
 * by client. The client also needs to provide the start and end time
 * (Local) of the period that it wants data for (if shorter
 * than the 7 days is wanted).  For the time-series product, the client
 * needs to provide an array of boolean values corresponding
 ...
 * to start retrieving data for and the number of days worth
 * of data.  The Unit will default to U.S. Standard (english) unless
 * Metric is chosen by client. The client also needs to provide
 ...
 * longitude pairs are substituted for the single latitude and longitude input.
 * Each latitude and longitude pair is composed of a latitude and longitude delimited
 * by a comma.  
 * 		  For the LatLonListSubgrid, the user provides a comma delimited
 * latitude and longitude pair for the lower left and for the upper right
 *  corners of a rectangular subgrid.
 *  
 * The function can also take a integer resolution to reduce the number of grid points returned.
 * The service then returns a list of latitude and longitude pairs for all the grid points
 * contained in the subgrid. weather values should appear in the time series
 * product.  
